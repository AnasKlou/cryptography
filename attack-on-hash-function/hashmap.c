...
struct hashmap {
    ...
    uint64_t seed0;
    uint64_t seed1;
    uint64_t (*hash)(const void *item, uint64_t seed0, uint64_t seed1,int index);
...
static uint64_t get_hash(struct hashmap *map, const void *key, int set_or_get) {
    return map->hash(key, map->seed0, map->seed1,set_or_get) << 16 >> 16;
}
...
void *hashmap_set(struct hashmap *map, const void *item) {
    ...
    entry->hash = get_hash(map, item,0);
    entry->dib = 1;
...
void *hashmap_get(struct hashmap *map, const void *key) {
    ...
    uint64_t hash = get_hash(map, key,1);
	...
	for (;;) {
        ...
        // {
        if (map->compare(key, bucket_item(bucket), map->udata) == 0)
        {
